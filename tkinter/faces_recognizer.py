...
#How it's done: 
#	1/ scaling down the image by a factor of "SCALER"
#	2/ doing the normal recognition process
#	3/ scaling up the image to the real size for display
#	4/ multiplying the coordinates of the faces found by the scaler
...
def create_face_encodings (frame): 
	...
	try:
		if frame is not None:
			locate_faces = face_recognition.face_locations(frame, model=MODEL, number_of_times_to_upsample=2) # finding all faces' locations in the frame
			face_encoded = face_recognition.face_encodings(frame, num_jitters=2) # creating the encodings for all found faces in the frame
			KNOWN_FACES_ENCODINGS.append(face_encoded) # populating the faces' encodings list
	except:
		pass
